...
_continue:
			...
			pmlmepriv->cur_network.join_res == _TRUE)
	{
		...
		if (psta == NULL)
		{
			...
			SPIN_LOCK_BH(psta->lock, &irqL);
			psta->state |= _FW_LINKED;
			SPIN_UNLOCK_BH(psta->lock, &irqL);
...
unsigned int OnAssocReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if(pstat && (pstat->state & WIFI_FW_ASSOC_SUCCESS) && (_STATS_SUCCESSFUL_==status))
	{
		...
		SPIN_UNLOCK_BH(pstat->lock, &irqL);
#endif //CONFIG_IOCTL_CFG80211
...
static void issue_p2p_GO_response(_adapter *padapter, u8* raddr, u8* frame_body,uint len, u8 result)
{
	...
	*(fctrl) = 0;
	...
	cpu_wps_devicepassword_id = be16_to_cpu( wps_devicepassword_id );
	...
	*(__be16*) ( wpsie + wpsielen ) = cpu_to_be16( 0x0002 );
	...
	//	Value:
	if ( cpu_wps_devicepassword_id == WPS_DPID_USER_SPEC )
	{
		*(__be16*) ( wpsie + wpsielen ) = cpu_to_be16( WPS_DPID_REGISTRAR_SPEC );
	}
	else if ( cpu_wps_devicepassword_id == WPS_DPID_REGISTRAR_SPEC )
	{
		*(__be16*) ( wpsie + wpsielen ) = cpu_to_be16( WPS_DPID_USER_SPEC );
	}
	else
...
#if defined (CONFIG_AP_MODE) && defined (CONFIG_NATIVEAP_MLME)
	SPIN_LOCK_BH(pmlmepriv->bcn_update_lock, &irqL);
#endif //#if defined (CONFIG_AP_MODE) && defined (CONFIG_NATIVEAP_MLME)
...
_issue_bcn:
	...
	SPIN_UNLOCK_BH(pmlmepriv->bcn_update_lock, &irqL);
#endif //#if defined (CONFIG_AP_MODE) && defined (CONFIG_NATIVEAP_MLME)
