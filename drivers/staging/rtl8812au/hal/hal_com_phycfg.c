...
#ifdef CONFIG_LOAD_PHY_PARA_FROM_FILE
	if (hal_file_name) {
		rtw_merge_string(phy_para_file_path, PATH_LENGTH_MAX, rtw_phy_file_path, hal_file_name);
		return rtw_is_file_readable(phy_para_file_path);
	}
#endif
	return _FALSE;
}
...
)
{
	...
	{
		rtw_merge_string(phy_para_file_path, PATH_LENGTH_MAX, rtw_phy_file_path, pFileName);

		if (rtw_is_file_readable(phy_para_file_path) == _TRUE)
		{
			rlen = rtw_retrieve_from_file(phy_para_file_path, pHalData->para_file_buf, MAX_PARA_FILE_BUF_LEN);
			if (rlen > 0)
			{
				rtStatus = _SUCCESS;
