...
static void tkip_mixing_phase1(const u8 *tk, struct tkip_ctx *ctx,
			       const u8 *ta, const u32 tsc_IV32)
{
	int i, j;
	u16 *p1k = ctx->p1k;
				 ...
				 struct sk_buff *skb, const u64 pn)
{
	...
	int offset = ieee80211_hdrlen(hdr->frame_control) +
		     IEEE80211_TKIP_IV_LEN;
	u8 *data = (u8 *)hdr + offset;
	size_t len = skb->len - offset - IEEE80211_TKIP_ICV_LEN;
	u32 iv32 = pn >> 16;
	u16 iv16 = pn;

	tkip_mixing_phase1(tk, &ctx, hdr->addr2, iv32);
	tkip_mixing_phase2(tk, &ctx, iv16, rc4key);
	ieee80211_wep_encrypt_data(tfm, rc4key, 16, data, len);
}
				...
				const u8 *tk, struct sk_buff *skb, u64 pn)
{
	...
	tkip_mixing_phase1(tk, &ctx, hdr->addr2, iv32);
	tkip_mixing_phase2(tk, &ctx, iv16, rc4key);
	return ieee80211_wep_decrypt_data(tfm, rc4key, 16, data, len);
}
